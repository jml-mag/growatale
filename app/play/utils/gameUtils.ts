...
export async function createScene(scene: Scene, previousPrimaryText: string, playerChoice: string) {
  let getPrompt = generatePrompt(scene, previousPrimaryText, playerChoice);
...
export function generatePrompt(scene: Scene, previousPrimaryText: string, playerChoice: string) {
    let sceneDescription: string;
    let backOption: string;
    if (scene.previous_scene !== "") {
      sceneDescription = scene.previous_scene;
      backOption = "Additionally, include an option to 'go back', allowing the player to return to the previous location. This should always be the final option in the list.";
    } else {
    ...
    const foundationPrompt = `You are ${writer}, writing a ${genre} text game in your literary style appropriate for a ${age} year old. The scene to write is based on ${sceneDescription} at ${time}. The story must be told from the perspective of the player in the first person. Never use 'I' or 'you' in the description. The scene is only from the eyes of the player. Do not describe the player in the scene, describe the scene as seen by the player. The previous scene text is: "${previousPrimaryText}", and the player's choice was: "${playerChoice}". Please return a detailed JSON object with keys for 'story', 'scene_description', and 'player_options'.`;
